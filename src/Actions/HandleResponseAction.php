...
class HandleResponseAction
{
    ...
    public function handle($httpcode, $response): array
    {
        if ((int)$httpcode === 200) {
            ...
            throw new Exception('Unauthorized zatca settings!');
        }
        else {
            if (is_array($response) && array_key_exists('code', $response)) {
                ...
                throw new Exception('Unhandled validation rules exception!');
            } else {

                throw new Exception('Unhandled zatca error exception!');
